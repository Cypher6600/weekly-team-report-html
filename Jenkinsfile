pipeline {
    ...
    stages {
       ...
       // stage('nmp-build') {
        ...
        sh "${sonarqubeScannerHome}/bin/sonar-scanner -e -Dsonar.host.url=http://${SONARQUBE_HOSTNAME}:9000 -Dsonar.login='admin' -Dsonar.password='Admin@123' -Dsonar.projectName=WebApp -Dsonar.projectVersion=${env.BUILD_NUMBER} -Dsonar.projectKey=GS -Dsonar.sources=src/ -Dsonar.tests=src/ -Dsonar.java.binaries=build/**/* -Dsonar.language=java"
                        }
                 }    
         }
  }
